...
当前进度：阶段 0–11 的迁移和自动验收已完成。隔离新世界已通过注册对象、设备规则、程序化飞船和四维度 RCON 烟测；两份只读复制的 Forge 旧存档已成功升级、保存并再次启动。真人验收发现并修复了 Payload 只更新网络镜像、未更新跃迁渲染状态的问题；完整构建现共运行 158 项测试，0 failures、0 errors、0 skipped，客户端资源重载和专用服务器启动日志无模组资源或注册错误。回船、行星旅行和跃迁现已使用真实维度；翠绿天体仍按玩法基线使用主世界表面。

发布前仍需真人实机确认 GUI Scale 1/2/3/4/Auto、真实点击与跃迁手感、登录/死亡/重进流程，以及单人和局域网交互；这些视觉与操作判断不由自动烟测代替。
...
验证玩家已改造的旧飞船备份时，如果固定坐标的控制台已被玩家移除，可显式附加 `-AllowMissingConsole`；新世界和普通验收不要使用该开关。
