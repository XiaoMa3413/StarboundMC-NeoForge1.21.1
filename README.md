...
- 新玩家首次登录会获得物质枪并被送往初始飞船；固定传送器位于飞船内，不再作为初始物品发放。
- 核心离线时会收到一次 N.O.V.A. 异常提示；未找到终端时最多补发一次定位提示。
- 首次抵达行星表面时先确认通讯链路，约 6 秒后提示探索与物质枪操作；首次拾取原木会收到资源反馈，并在稍后触发一次共享轨道矿物扫描。
...
- 物质枪组件由上、下、左、右四个铁锭围绕中央青金石合成，一次获得 2 个。
...
- 默认测试套件：252 项，0 failures、0 errors、0 skipped。
