...
实施状态（2026-08-24）：**连续宇宙与服务器权威跃迁迁移已完成**。

- 已接回 `UniverseRouteFrame`、`ShipSpace`、`ShipFlightController`、`ShipWarpManager` 与星图数据模型，保留 12 条有向确定性航线、天体安全半径绕行、11–28 秒时长、同星系 20/跨星系 100 燃料成本，以及每 5 tick/阶段变化同步策略。
- 新增 `Stage7ServerPayloadActions`；只有仍在有效飞船控制台菜单内、位于真实飞船维度、目标可达且不同于当前天体、燃料充足时，服务端才会启动跃迁。阶段 8 注册飞船维度前不会弱化该权限边界。
- 服务端启动、停止、tick、玩家登录及换维度生命周期已迁移到 NeoForge 事件。飞行状态继续持久化；服务端修订号以世界时间初始化，客户端在登录/登出边界清空镜像，避免重启或重连后的旧快照压制新状态。
- 修复最短 220 tick 月球航线跳过 `CRUISE` 的边界问题，在不改变总时长的前提下保留至少 1 tick 巡航；落地后速度显式归零。
- 当前 75 项测试全部通过，完整 `test build` 成功；专用服务器启动至 `Done`，确认跃迁生命周期事件可在无客户端类加载错误的环境中订阅。真实维度内的同星系、跨星系和恢复实机回归将在阶段 8 注册维度后继续。
...
实施状态（2026-08-24）：**迁移交付完成；自动验收与跃迁真人复验已通过**。
...
- 真人跃迁验收随后发现服务端已显示“开始跃迁”，但客户端画面不推进。根因是阶段 3 Payload 迁移只更新了 `ClientNetworkState`，而星图、空间渲染和声音仍读取 `ClientPlanetState`。客户端 Payload handler 现已恢复当前天体、跃迁开始、燃料、星图访问状态和权威飞行快照的渲染状态副作用；默认测试源集也已纳入此前被遗漏的 `network` 包测试。修复后完整构建共执行 158 项测试，0 failures、0 errors、0 skipped；专服再次启动至 `Done` 并正常保存全部维度后停服。
- 修复后的同星系/跨星系跃迁、燃料扣除、航行动画和抵达状态已由用户真人复验通过。GUI Scale 1/2/3/4/Auto、登录/死亡/重进，以及单人、局域网和不同模组组合仍应作为后续 Alpha 版本的持续回归矩阵；自动验收不能替代这些视觉与操作判断。
...
## 12. 迁移期新对话建议首条消息（历史记录）

> 以下模板仅记录迁移期间的交接方式。迁移已经完成，当前任务应从
> [`../README.md`](../README.md) 的活动计划入口开始。

```text
请完整阅读仓库中的 NeoForge 迁移计划，并把它作为本次迁移的目标与约束来源。
...
- [x] 完成客户端渲染和星图迁移。
- [x] 完成资源和 datagen 迁移。
- [x] 完成新世界、旧存档备份、专用服务器自动验收和跃迁真人复验；迁移版本可交付。
