...
> **归档状态：已完成。** 需求已实现并通过自动测试及客户端视觉、交互和跃迁核验；矿物
> 分布按产品决定延期，待行星资源生成模型落地后另立计划。
...
- 每颗恒星代表一个恒星系。
- 恒星系之间用虚线超空间航道连接。
- 超空间航道使用连续的浮点线段和均匀间隔的短 dash，带低亮度光晕，不使用方形点阵。
- 选中恒星后显示恒星信息板，包括名称、类型、描述和该恒星系的天体数量。
- 单击恒星只选中并展示信息板；双击恒星和信息板中的“进入恒星系”按钮都进入
  对应的恒星系视图。
- 深空星域右键不关闭界面；`Esc` 关闭星图。
- 星域拓扑由 `assets/starboundmc/starmap/galaxy_graph.json` 配置。`nodes` 通过稳定
  `system_id` 引用服务端星系定义并配置归一化位置；`routes` 使用显式稳定 ID 和
  `from`/`to` 端点，不允许依赖节点数组顺序隐式连线。
- 节点和航道都保留 `unlocked`、`reachable` 字段。资源缺失或校验失败时使用内置
  显式回退图；锁定或不可达的恒星仍可查看信息，但不能进入对应恒星系。

### 2.2 恒星系

- 中央显示恒星，周围绘制行星轨道和行星。
- 中央恒星可以单独选中并展示恒星信息板；该信息板不显示跃迁按钮。
- 行星沿轨道公转。
- 公转轨道使用连续的浮点圆环；卫星从属轨道可用低亮度短 dash 区分。
...
- 当前客户端预览使用每 tick `0.0035` 弧度作为 52 像素参考轨道的基准速度，
  轨道速度按 `sqrt(52 / orbitRadius)` 衰减，外圈行星因此比内圈明显更慢；
  卫星在自身轨道速度上使用 1.45 倍系数，并与绘制和命中测试共用同一相位模型。
