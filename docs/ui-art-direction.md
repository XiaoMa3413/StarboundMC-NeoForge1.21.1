...
打印站最大的视觉重点应是**制造**，而不是配方管理。

推荐结构：左侧配方导航、中央制造舱/全息预览、右侧材料与状态、底部数量和 PRINT 操作。中央制造物预览应该是页面第一视觉主体。

制造中的物品至少可表现三个阶段：轮廓/WIRE → 半透明 HOLOGRAM → MATERIALIZED。打印过程应成为视觉事件，而不是只有进度条变化。
