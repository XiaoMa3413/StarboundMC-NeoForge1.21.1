...
public class PlanetRenderer
{
                                                  ...
                                                  float shipYaw, float shipPitch, float animationTicks)
    {
        ...
        if (shader != null)
        {
            ...
            drawPlanetSphereGpu(pose, matrix, body, cx, cy, cz, scale, worldSun, alpha,
                    ...
                    textureOf(body), false, 0.0F, 0.0F, shader);
            return;
        }
     ...
     * planet surface and the procedural cloud layer both draw through it.
     *
     ...
     *
     * <p>In cloud mode the shader replaces the albedo with procedural density
     * on the sphere's own frame, so the layer needs no texture and inherits the
     * very same terminator maths as the surface below it.</p>
     */
    private static void drawPlanetSphereGpu(PoseStack pose, Matrix4f matrix, CelestialBodyDefinition body,
                                            ...
                                            ResourceLocation texture, boolean cloudMode,
                                            ...
                                            ShaderInstance shader)
    {
        ...
        Uniform cloudModeUniform = shader.getUniform("CloudMode");
        if (cloudModeUniform != null)
            cloudModeUniform.set(cloudMode ? 1.0F : 0.0F);
        Uniform cloudCoverageUniform = shader.getUniform("CloudCoverage");
        if (cloudCoverageUniform != null)
            cloudCoverageUniform.set(cloudCoverage);
        Uniform cloudTimeUniform = shader.getUniform("CloudTime");
        if (cloudTimeUniform != null)
            cloudTimeUniform.set(cloudTime);
    ...
    /**
     * Draws the body's cloud layer, if it authors one: a slightly larger sphere
     * whose density is procedural, rotating at its own rate so the weather
     * drifts across the ground. Lighting and geometry are shared with the
     * planet surface, so the terminator stays aligned and no extra shader or
     * mesh exists.
     */
                                       ...
                                       float shipYaw, float shipPitch, float animationTicks)
    {
        ...
        drawPlanetSphereGpu(pose, pose.last().pose(), body, cx, cy, cz,
                ...
                null, true, cloud.coverage(), animationTicks, shader);
    }
