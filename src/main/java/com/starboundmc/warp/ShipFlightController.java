...
public final class ShipFlightController
{
    ...
    private static FlightRoute buildRoute(Planet from, Planet to)
    {
        ...
        poly = densifyNearGasGiant(frame, poly);
        ...
        liftRouteOverGasGiantRings(frame, pts, cum, total);
        return new FlightRoute(frame, ShipSpace.universeDock(to), pts, cum, total);
    }

    /**
     * Subdivides any corridor segment that reaches the gas giant's ring disc so
     ...
     */
    private static List<Vec3> densifyNearGasGiant(UniverseRouteFrame frame, List<Vec3> poly)
    {
        Planet giant = Planet.GAS_GIANT;
        Vec3 center = frame.toRelative(ShipSpace.universeBodyPosition(giant));
        double reach = GasGiantGeometry.ringOuterDistance(ShipSpace.radius(giant))
                + ShipSpace.radius(giant) * 0.5;
    ...
    /**
     * Lifts the corridor's mid-section out of the gas giant's ring plane. The
     ...
     */
    private static void liftRouteOverGasGiantRings(UniverseRouteFrame frame, Vec3[] pts,
                                                   double[] cum, double total)
    {
        ...
        Planet giant = Planet.GAS_GIANT;
        Vec3 center = frame.toRelative(ShipSpace.universeBodyPosition(giant));
        double radius = ShipSpace.radius(giant);
        ...
        // The hump always lifts to the side the giant's dock already leans
        ...
        double departureOffset = GasGiantGeometry.planeOffset(
                ShipSpace.vDock(giant), ShipSpace.qPos(giant));
