...
class ExposureRulesTest {
    @Test void anyCorrespondingModuleRecoversEvenLegacyHighTierExposure() {
        ...
        assertEquals(75, ExposureRules.step(80, 3, 1, 2, 5).exposure());
        assertFalse(ExposureRules.step(100, 3, 1, 2, 5).damage());
    }
    @Test void legacyTemperatureLevelsDoNotMultiplyAccumulation() {
        assertEquals(2, ExposureRules.step(0, 1, 0, 2, 5).exposure());
        assertEquals(2, ExposureRules.step(0, 3, 0, 2, 5).exposure());
